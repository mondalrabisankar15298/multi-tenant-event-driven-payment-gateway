...
async def _sync_merchant_created(pool, schema, payload):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            datetime.fromisoformat(str(payload.get("created_at"))) if payload.get("created_at") else datetime.utcnow(),
        )
...
async def _sync_customer_created(pool, schema, payload):
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            datetime.fromisoformat(str(payload["created_at"])) if payload.get("created_at") else datetime.utcnow(),
            datetime.fromisoformat(str(payload["updated_at"])) if payload.get("updated_at") else datetime.utcnow(),
        )
...
async def _sync_customer_updated(pool, schema, payload):
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                f"UPDATE {schema}.payments SET customer_name = $1, customer_email = $2 WHERE customer_id = $3",
            ...
            await conn.execute(
                ...
                WHERE payment_id IN (SELECT payment_id FROM {schema}.payments WHERE customer_id = $2)
                """,
...
async def _sync_customer_deleted(pool, schema, payload):
    async with pool.acquire() as conn:
        await conn.execute(
            f"DELETE FROM {schema}.customers WHERE customer_id = $1",
            payload["customer_id"],
        )
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                float(payload["amount"]), payload.get("currency", "INR"),
            ...
            await conn.execute(
                f"UPDATE {schema}.customers SET total_payments = total_payments + 1 WHERE customer_id = $1",
                payload["customer_id"],
            )
...
async def _sync_payment_captured(pool, schema, payload):
    async with pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute(
                f"UPDATE {schema}.payments SET status = $1, updated_at = $2 WHERE payment_id = $3",
                "captured",
                datetime.fromisoformat(str(payload["updated_at"])) if payload.get("updated_at") else datetime.utcnow(),
                payload["payment_id"],
            )
            ...
            await conn.execute(
                f"""
                UPDATE {schema}.customers
                SET total_spent = total_spent + $1, last_payment_at = NOW()
                WHERE customer_id = $2
                """,
                float(payload["amount"]), payload["customer_id"],
            )
...
async def _sync_payment_failed(pool, schema, payload):
    async with pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute(
                f"UPDATE {schema}.payments SET status = 'failed', failure_reason = $1, updated_at = $2 WHERE payment_id = $3",
...
async def _sync_payment_authorized(pool, schema, payload):
    async with pool.acquire() as conn:
        await conn.execute(
            f"UPDATE {schema}.payments SET status = 'authorized', updated_at = $1 WHERE payment_id = $2",
            datetime.fromisoformat(str(payload["updated_at"])) if payload.get("updated_at") else datetime.utcnow(),
            payload["payment_id"],
        )
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            payment = await conn.fetchrow(
                f"SELECT amount, customer_name FROM {schema}.payments WHERE payment_id = $1",
                payload["payment_id"],
            )
            ...
            await conn.execute(
                ...
                float(payload["amount"]), payload.get("reason"), payload["status"],
                datetime.fromisoformat(str(payload["created_at"])) if payload.get("created_at") else datetime.utcnow(),
                datetime.fromisoformat(str(payload["updated_at"])) if payload.get("updated_at") else datetime.utcnow(),
            )
            ...
            if payload.get("customer_id"):
                await conn.execute(
                    f"UPDATE {schema}.customers SET total_spent = total_spent - $1 WHERE customer_id = $2",
                    float(payload["amount"]), payload["customer_id"],
                )
...
async def _sync_refund_processed(pool, schema, payload):
    async with pool.acquire() as conn:
        await conn.execute(
            f"UPDATE {schema}.refunds SET status = 'processed', updated_at = $1 WHERE refund_id = $2",
            datetime.fromisoformat(str(payload["updated_at"])) if payload.get("updated_at") else datetime.utcnow(),
            payload["refund_id"],
        )
